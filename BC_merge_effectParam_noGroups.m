...
% filenames 
directory = '/panfs/panasas01/bisc/ss16191/Forschung/Programme/OutgroupConflict_FawcettRadford/EffectParameter_noGroups/Output/';
seedFilename = '../listOfRandomSeeds_100simulations';
filename = 'populationOverTime_noGroups_';
outputFilename = 'mergedResults_effectParameter_noGroups';
...
%% parameters
noAttractors = 9;
noGroupsArray = [10,20,30,50,75,100,200,300,400,500];
noParameter = length(noGroupsArray);
aportCond = false;

%% load initial file to extract parameters
resultMat = load([string(strcat(directory,filename,num2str(noGroupsArray(1)),'_',seeds{1}(1)))]);
...
for param =1:noParameter
  disp(['noGroups value = ' num2str(noGroupsArray(param))])
  %% loop through each output file and record mean(w) and mean(a)
  for i=1:noSimulationPerParameter
    disp(['Sim run = ' num2str(i)]);

    clear('resultMat');
    fn = string(strcat(directory,filename,num2str(noGroupsArray(param)),'_',seeds{1}(i)));
    resultMat = load(fn);
