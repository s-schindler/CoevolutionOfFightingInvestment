...
//for(int i=0; i<pop.getNoGroups(); i++)System.out.println(cumFitnessArray[i]+"\t"+pop.getGroups().get(i).getFitness());

	// iterate through list
        for(int i=0; i< Population.getNoGroups(); i++){
	    //get random number
	    double rand = randomGenerator.nextDouble()*sumFitness;
//System.out.println("rand="+rand);
	    ...
	    do{
		count++;
	    } while (cumFitnessArray[count]<rand && count<Population.getNoGroups()-1);
	    newListOfGroups.add(new Group(pop.getGroups().get(count).getW(),pop.getGroups().get(count).getA()));
//System.out.println(count+" added "+sumFitness);
	}
