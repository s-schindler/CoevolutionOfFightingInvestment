...
public class Population {
    ...
    static final int noParameters = 7;
	...
	double probToWinForAttackers = 1/(1+Math.exp(-param_k*(w*a-(wdf+param_h))));
	double probToWinForDefenders = 1/(1+Math.exp(-param_k*((w*(1-a)+param_h)-waf)));
	double baselineSurvival = 1 - param_mu;
	double investmentCosts = w*(param_cmu + param_cm*a*w); 
	double engagementCostsForAttackers = Math.max(0,(1-(w*a-(wdf+param_h)))*param_ca);
	double engagementCostsForDefenders = Math.max(0,(1-((w*(1-a)+param_h)-waf))*param_cd);
	...
	// gaining none, possibly looing one
